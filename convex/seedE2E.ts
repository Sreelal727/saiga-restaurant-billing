/**
 * E2E test fixture. This is the OLD Saiga demo dataset, kept only so the
 * Playwright golden-path test can hit a deterministic menu / table / staff
 * layout. It is **gated by the SEED_ENABLED environment variable** — on a
 * production deployment that env var is unset, so this mutation refuses to
 * run even if someone calls it directly.
 *
 * Real data is entered through the admin UI. Do not call this from the app.
 */
...
export const run = mutation({
  args: {},
  handler: async (ctx) => {
    if (process.env.SEED_ENABLED !== "yes") {
      throw new Error(
        "Seeding is disabled on this deployment. Set SEED_ENABLED=yes in the " +
          "Convex env vars (dev/test only — never on prod)."
      );
    }

    // Clear existing data so the suite always starts from a known shape.
    for (const table of [
      "restaurant_settings",
      "restaurant_tables",
      "menu_categories",
    ...
    return { success: true, message: "E2E fixture loaded" };
  },
});
